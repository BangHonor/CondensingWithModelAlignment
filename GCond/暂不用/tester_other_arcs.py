...
#一直都是用SGC+GCN获得小图，只不过这里不只是用GCN进行fit，而是用多种模型进行fit_with_val
class Evaluator:
    ...
    def test(self, nlayers, model_type, verbose=True):
        ...
        feat_syn, adj_syn, labels_syn = self.get_syn_data(model_type)#得到凝聚之后的图
    ...
    def train(self, verbose=True):#table 3 入口
        args = self.args
        data = self.data
        ...
        for model_type in ['GCN',  'GraphSage', 'SGC1', 'MLP', 'APPNP1', 'Cheby']:#每一个模型用于凝聚后的图
            res = []
            nlayer = 2
            for i in range(runs):
