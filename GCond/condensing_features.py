...
from models.monet import MoNet as MoNet_PYG
from models.gatedgnn import GatedGNN as GatedGNN_PYG, ResGatedGCN as ResGatedGCN_PYG
...
parser.add_argument('--lr_model', type=float, default=0.001)#GCN:0.01 SGC:0.01 GIN:0.02 SAGE:0.01 JKNET:0.01 GATED:0.001
...
parser.add_argument('--student_model_loop', type=int, default=2000)#GCN:2000 SGC:500 GIN:10000 SAGE:5000 JKNET:500 GATED:500
parser.add_argument('--threshold', type=float, default=0.1, help='adj threshold.')
parser.add_argument('--kl_t', type=float, default=2, help='distillation temperature.')#GCN:2 SGC:0.5(2差0.14) GIN:2 SAGE:2 JKNET:2
...
if __name__ == '__main__':
    ...
    if args.model=='GCN':
        ...
        model=MoNet_PYG(nfeat=data.feat_train.shape[1], nhid=args.hidden, nclass=data.nclass, nlayers=args.nlayers, dropout=0, kernel=3, dim=1, with_bias=True, norm=True, device=device).to(device)
    elif args.model=='JKNet':
        model=JKNet_PYG(nfeat=data.feat_train.shape[1], nhid=args.hidden, nclass=data.nclass, dropout=0, nlayers=args.nlayers+1, norm='BatchNorm', cached=True, jk='cat').to(device)
    elif args.model=='GatedGNN':
        model=GatedGNN_PYG(nfeat=data.feat_train.shape[1], nhid=args.hidden, nclass=data.nclass, dropout=0, nlayers=args.nlayers, norm='BatchNorm', cached=True).to(device)
    elif args.model=='GatedGCN':
        model=ResGatedGCN_PYG(nfeat=data.feat_train.shape[1], nhid=args.hidden, nclass=data.nclass, dropout=0, nlayers=args.nlayers, norm='BatchNorm', cached=True).to(device)
    else:
        model=MLP_PYG(channel_list=[data.feat_train.shape[1],args.hidden,args.hidden,data.nclass], in_channels=data.feat_train.shape[1], hidden_channels=args.hidden, out_channels=data.nclass, dropout=[0,0,0], num_layers=args.nlayers, norm='BatchNorm', cached=True).to(device)
