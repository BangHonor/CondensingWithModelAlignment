...
class GCond:
    ...
    def train(self, verbose=True):#开始训练
        ...
        self.adj_syn=torch.where(adj_sub > 0, 1, 0).to(self.device)
        self.adj_syn=self.adj_syn.float()
        self.adj_syn.requires_grad_()
        ...
        for it in range(args.epochs+1):
            ...
            for ol in range(outer_loop):
                ...
                adj_syn_norm = utils.normalize_adj_tensor(self.adj_syn, sparse=False)
                ...
                # TODO: regularize
                if args.alpha > 0:
                    loss_reg = args.alpha * regularization(adj_syn_norm, utils.tensor2onehot(labels_syn))
                else:
