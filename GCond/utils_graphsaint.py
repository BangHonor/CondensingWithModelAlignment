...
class DataGraphSAINT:#input a dataset and return full data
    ...
    def __init__(self, dataset, **kwargs):#**kwargs 允许你将不定长度的键值对, 作为参数传递给一个函数。
        dataset_str='/home/xzb/GCond/data/'+dataset+'/'
        adj_full = sp.load_npz(dataset_str+'adj_full.npz')#easier to partition
        ...
        print("the size of train graph:",self.adj_train.shape)
        print("the size of val graph:",self.adj_val.shape)
        print("the size of test graph:",self.adj_test.shape)
