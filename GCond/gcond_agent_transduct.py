...
class GCond:
        ...
        for it in range(args.epochs+1):
            ...
            for ol in range(outer_loop):
                ...
                for c in range(data.nclass):#不同的类逐个分析
                    ...
                    loss_real = F.nll_loss(output, labels_train)
                    gw_real = torch.autograd.grad(loss_real, model_parameters)#作用：计算并返回outputs对w的梯度
                    gw_real = list((_.detach().clone() for _ in gw_real))#通过 tensor.detach().clone() 操作生成一个和原本 tensor 值相同的新 tensor 但是没有用到计算图所以节省了内
                    gw_real_list.append(gw_real)
                    ...
                    loss_syn = F.nll_loss(
                            output_syn[ind[0]: ind[1]],
                            labels_syn[ind[0]: ind[1]])
                    gw_syn = torch.autograd.grad(loss_syn, model_parameters, create_graph=True)#要计算高阶导要create_graph，不然requires_grad为false
                    
                    coeff = self.num_class_dict[c] / max(self.num_class_dict.values())#这个类loss的权重
                    matching_loss=match_loss(gw_syn, gw_real, args, device=self.device)
                    mean_syn=self.feat_syn[ind[0]: ind[1]].mean(dim=0)
                    mean=feats_train.mean(dim=0)
                    mseloss=nn.MSELoss()
                    aligning_loss=mseloss(mean_syn,mean)
                    loss += coeff*(matching_loss+500*aligning_loss)#梯度匹配算法 
                ...
                # else:
                self.optimizer_feat.step()
                for gw_real in gw_real_list:
                    model_parameters=[a-args.lr_model*b for a,b in zip(model_parameters,gw_real)]
                i=0
                for para in model.parameters():
                    para.data=model_parameters[i]
                    i=i+1
                model_parameters = list(model.parameters())
            ...
            if verbose and it%100==0:
                # x_syn = self.feat_syn.detach().cpu().numpy()
                # y_syn = labels_syn.cpu().numpy()
                # x_tsne_syn = TSNE(n_components=2).fit_transform(x_syn)

                # plt.figure(figsize=(8,8))
                # plt.scatter(x_tsne[:, 0], x_tsne[:, 1])
                # plt.scatter(x_tsne_syn[:, 0], x_tsne_syn[:, 1])
                # plt.savefig(f'/home/disk3/xzb/GCond/pictures/{args.dataset}_{it}.png')
    ...
    def test_with_val(self, features, adj, best_acc, verbose=True):#test    
        ...
        #     torch.save(adj_syn, f'/home/disk3/xzb/GCond/saved_ours/adj_{args.dataset}_{args.model}_{args.reduction_rate}_{args.seed}.pt')#save X' A'
        ...
        #     torch.save(model.state_dict(), f'/home/disk3/xzb/GCond/saved_model/{args.model}_{args.dataset}_{args.reduction_rate}_{args.seed}.pt') 
